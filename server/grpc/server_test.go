...
import (
	...
	clientSpy "github.com/jrobison153/raft/test/doubles/policy/client"
	"reflect"
	"strings"
	"testing"
)
...
func setup(spy *clientSpy.Spy) *RaftServer {

	server := New()

	if spy != nil {
		server.SetClientPolicy(spy)
	}
...
func TestWhenCreatingTheServerThePolicyClientIsDefaulted(t *testing.T) {

	server := setup(nil)

	typeOfExpectedPolicyClient := reflect.TypeOf(&client.Client{}).String()
	typeOfPolicyClient := reflect.TypeOf(server.policyClient).String()

	if strings.Compare(typeOfExpectedPolicyClient, typeOfPolicyClient) != 0 {
		t.Errorf("Default value for policy client should have been %v but was %v",
			typeOfExpectedPolicyClient,
			typeOfPolicyClient)
	}
}

func TestWhenThePolicyClientIsOverriddenThenTheNewImplIsSet(t *testing.T) {

	clientPolicySpy := clientSpy.New()

	server := setup(clientPolicySpy)

	typeOfExpectedPolicyClient := reflect.TypeOf(clientPolicySpy).String()
	typeOfPolicyClient := reflect.TypeOf(server.policyClient).String()
...
func TestWhenDefaultPolicyClientIsUsedThenCorrectLoggerImplIsAlsoDefaulted(t *testing.T) {

	server := setup(nil)

	typeOfExpectedLogger := reflect.TypeOf(&log.ArrayLog{}).String()
...
func TestWhenDefaultPolicyClientIsUsedThenCorrectReplicatorImplIsAlsoDefaulted(t *testing.T) {

	server := setup(nil)

	typeOfExpectedReplicator := reflect.TypeOf(&replicator.Quorum{}).String()
	typeOfActualReplicator := server.policyClient.TypeOfReplicator()
